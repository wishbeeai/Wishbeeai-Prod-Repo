...
export function RefundDirectSummaryContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border-2 border-[#DAA520]/20 bg-white p-6 space-y-4">
        ...
        {(bank > 0 || credits > 0) && giftId && (
          <p className="text-xs font-medium text-[#8B5A3C]/90" data-testid="gift-status">
            Gift status: SETTLED_REFUND
          </p>
        )}
        <ul className="space-y-2 text-sm text-[#8B5A3C]/90">
          <li>
      ...
      <Link
        href="/gifts/active"
        className="inline-flex items-center gap-2 text-sm font-medium text-[#8B5A3C] hover:text-[#6B4423]"
      >

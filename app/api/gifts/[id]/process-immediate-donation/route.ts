...
import { sendDonationReceiptEmail } from "@/lib/donation-receipt-email"
 ...
 * Processes an immediate charity donation and sends receipt email on success.
 * On failure, marks gift_settlement as 'failed' and returns error.
 *
 * Body: {
 *   amount: number
 *   charityId: string
...
export async function POST(
  ...
  try {
    ...
    if (isNaN(amount) || amount <= 0 || !b.charityId || !b.charityName) {
      return NextResponse.json(
        { error: "amount, charityId, and charityName are required" },
        { status: 400 }
      )
    }
    ...
    // Verify gift exists and get user
    ...
    // Create settlement record with status='failed' initially; update to 'completed' on success
    const insertPayload = {
      gift_id: giftId,
      amount: netToCharity,
      disposition: "charity",
    ...
    if (insertError || !settlement) {
      console.error("[process-immediate-donation] Insert error:", insertError)
      return NextResponse.json(
    ...
    if (!result.success) {
      // Settlement stays 'failed'; user can retry
      return NextResponse.json(
    ...
    if (RESEND_API_KEY && organizerEmail) {
      try {
        ...
        console.error("[process-immediate-donation] Email error:", e)
        // Non-blocking; donation succeeded
      }
    }
    ...
    if (RESEND_API_KEY) {
      try {
        ...
        console.error("[process-immediate-donation] Recipient notification check error:", e)
      }
    }
    ...
    console.error("[process-immediate-donation]", err)
    return NextResponse.json(
